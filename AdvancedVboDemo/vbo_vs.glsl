...
void main(void)
{
	vec4 transformed_pos = instance_matrix * vec4(pos_attrib, 1.0);
	gl_Position = PV*M*transformed_pos; //transform vertices and send result into pipeline
	...
	outData.pw = vec3(M * transformed_pos);		//world-space vertex position
	outData.nw = vec3(M*vec4(normal_attrib, 0.0));	//world-space normal vector
	outData.color = instance_color;
}