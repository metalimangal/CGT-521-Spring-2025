...
{   
   ...
   vec4 ambient_term = ka*ktex*La*clear_color;
   ...
   float atten = 1.0;// no attenutation
   ...
   //height-based fog
   float f = max(0.0, 2.0*inData.pw.y);
   float height_fog_factor = 1.0-exp(-f*f);
   fragcolor = mix(fog_color, fragcolor, height_fog_factor);

   //depth fog
   float deye = 0.031*abs(inData.peye.z);
   float depth_fog_factor = exp(-deye*deye);
   fragcolor = mix(clear_color, fragcolor, depth_fog_factor);
}
